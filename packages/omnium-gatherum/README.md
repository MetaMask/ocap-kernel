...
const { capletId } = await omnium.caplet.install(manifest);

// 3. Get the caplet's root as an E()-usable presence
const echoRoot = await omnium.caplet.getRoot(capletId);

// 4. Call the echo method
