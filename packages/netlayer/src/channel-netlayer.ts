 ...
 * the provider was constructed with). The provider owns identity, so the
 * netlayer's `peerId` is `provider.peerId`.
 *
...
export function makeChannelNetlayer(params: {
  ...
  // Return the netlayer: sender + stop handle + connection management functions.
  return harden({
