...
// Import the engine under test - must be after mocks are set up.
let makeChannelNetlayer: typeof import('./channel-netlayer.ts').makeChannelNetlayer;

/**
 * Options accepted by the {@link initTransport} test shim: the engine options
 * plus the provider-only fields some call sites still pass (ignored here — the
 * provider is a mock).
 */
type TransportTestOptions = ChannelNetlayerOptions & {
  relays?: string[];
  directTransports?: unknown[];
  allowedWsHosts?: string[];
  maxQueue?: number;
};

/**
 * Map the loose test options bag onto the engine's {@link ChannelNetlayerOptions}.
 *
 * @param options - The options bag passed to the shim.
 * @param localIncarnationId - The incarnation id passed positionally.
...
/**
 * Thin shim preserving the historical `initTransport` call signature used
 * throughout this suite. It builds the engine over the mock {@link ChannelProvider}
 * (`mockConnectionFactory`) with a fresh {@link AbortController}, mirroring what
 * ocap-kernel's thin `initTransport` wrapper does around a real provider.
 *
 * @param _keySeed - Unused; identity lives on the provider now.
 * @param options - Engine + provider options (provider fields ignored).
 * @param handleMessage - Inbound message handler.
 * @param onRemoteGiveUp - Optional give-up callback.
 * @param localIncarnationId - Optional local incarnation id.
 * @param onIncarnationChange - Optional incarnation-change callback.
 * @returns The built netlayer.
 */
async function initTransport(
  ...
  return makeChannelNetlayer({
    ...
    provider: mockConnectionFactory as unknown as ChannelProvider,
    hooks: { handleMessage, onRemoteGiveUp, onIncarnationChange },
    options: toEngineOptions(options, localIncarnationId),
    logger: new Logger(),
    stopController,
  });
}
...
describe('makeChannelNetlayer', () => {
  // Import after all mocks are set up
  beforeAll(async () => {
    const networkModule = await import('./channel-netlayer.ts');
    makeChannelNetlayer = networkModule.makeChannelNetlayer;
  });
  ...
  describe('handshake protocol', () => {
    ...
    it('closes the dialed channel and throws PeerRestartedError when outbound handshake reports kernel-detected restart', async () => {
      const localIncarnationId = 'local-incarnation';
      // Kernel verdict says "yes, restart" — overrides PSM's first-contact
      // verdict and must trigger the close-without-register path.
      ...
      // The throw must happen AFTER closeChannel and the channel must
