/**
 * Daemon communication layer for the demo-display server. Spawns
 * `ocap daemon` subcommands and returns parsed results.
 *
 * Intentionally duplicates the small helper used by the openclaw
 * plugins (see `packages/agentmask/openclaw-plugin-discovery/daemon.ts`)
 * rather than importing it, so that demo-display talks to the matcher
 * through the same public OCAP-CLI surface an external consumer would
 * use — keeping a future repo split cheap.
 */
 ...
 * @param options.args - CLI arguments after the binary.
...
/**
 * Throw if the CLI exited non-zero.
 *
 * @param label - Operation name for the error message.
 * @param result - The collected CLI output and exit code.
 */
 ...
 * @param options.cliPath - Path to the ocap CLI entry point (.mjs).
 * @param options.ocapHome - Optional `--home` directory passed to every
 *   spawned `ocap` invocation. Use a distinct home (e.g.,
 *   `~/.ocap-consumer`) to address a daemon other than the default `~/.ocap`.
...
export function makeDaemonCaller(options: {
  ...
  const homeArgs: string[] =
    ocapHome !== undefined && ocapHome.length > 0 ? ['--home', ocapHome] : [];
