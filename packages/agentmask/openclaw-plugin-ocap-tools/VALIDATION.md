# Phase 3 validation — `@openclaw/ocap-tools`

This document walks through end-to-end validation of the plugin's
discovery tools against a running Phase 1 provider (MetaMask
extension) and a
...
## Stage A — Install the ocapTools plugin
...
```bash
openclaw config set 'plugins.entries.ocapTools.config.ocapHome' "$HOME/.ocap-consumer"
openclaw config set 'plugins.entries.ocapTools.config.matcherUrl' 'ocap:…'
```

(The path is `plugins.entries.<plugin-id>.config.<key>` — the
`plugins.ocapTools.<key>` shorthand does not exist in the openclaw
config schema.)
