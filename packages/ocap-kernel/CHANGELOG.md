...
- A persisted vat whose code can no longer be loaded no longer makes the whole kernel unbootable ([#964](https://github.com/MetaMask/ocap-kernel/issues/964))
  ...
  - The skipped vat's persisted record is kept rather than pruned, so a vat whose code becomes reachable again is restored by a later boot. Whether an unrestorable vat should instead take its subcluster down with it is left to the subcluster lifecycle ([#979](https://github.com/MetaMask/ocap-kernel/issues/979))
