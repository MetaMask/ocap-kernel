...
- A delivery whose endpoint has vanished is dropped only where the endpoint is gone for good, and `notify` and `bringOutYourDead` no longer take the run loop down when it has ([#1010](https://github.com/MetaMask/ocap-kernel/pull/1010))
