...
- **SECURITY:** A vat's `fetch` no longer follows a redirect out of its `network.allowedHosts`. The caveat saw only the pre-flight URL, so an allowed host answering `302 Location: http://169.254.169.254/...` sent the vat's request to a host it was never granted and handed it the response body — and an allowlist may well name a host the vat itself controls, so this is a general escape rather than a third-party risk ([#1026](https://github.com/MetaMask/ocap-kernel/pull/1026))
  - Every hop is now checked, and the vat cannot opt out: a `redirect` of `follow` is overridden rather than merged, on `init` and on a `Request` alike. A vat asking for `manual` or `error` is asking for less than the guarded walk and is obeyed. A `dispatcher` in `init`, which undici honours in place of the transport, is refused outright — dropping it would fall back to the global transport and egress anyway
  - A hop that stays inside the allowlist is followed as `fetch` (on undici) would have, and `response.redirected` still reports the chain
  - A hop that keeps the request body — every 307 and 308, and a 301 or 302 from anything but a POST — fails unless the body is one the kernel still holds as a value. A `Request`'s body is always a stream, so a `Request` carrying one fails here where `fetch` would have replayed it; the error names the hop and the reason
  - A hop to a scheme `fetch` will not follow (`data:`, `file:`, `javascript:`) is refused by name, and a runtime that hides a manual redirect's target — a browser, which answers with an opaque response — fails the fetch rather than handing the vat something that silently is not what it asked for
  - Not addressed: the caveat matches the hostname a vat names, so an allowlisted name that resolves to a loopback or link-local address still reaches it. Nor is a refused hop logged; the vat sees the error, and may swallow it
- **SECURITY:** The URL a vat's `fetch` requests is now always the URL its `network.allowedHosts` caveat approved. The allowlist and the `file:` prohibition were checked against one resolution of the caller's input, and then the input itself was forwarded to `fetch`, which resolved it a second time — so an input that answered differently on each read was validated as an allowed host and requested as a forbidden one (CWE-367). A vat holding `fetch` for a single host could reach any host, including loopback and cloud metadata endpoints ([#1026](https://github.com/MetaMask/ocap-kernel/pull/1026))
  - The input is resolved exactly once and replaced with a stand-in the vat cannot reach, so the caveat's verdict and the request's destination cannot diverge. An input that does answer differently on a second read is now refused outright rather than serviced at whichever URL it showed first
  - A `Request` is copied and then rebuilt around the URL that was checked, keeping the vat's method, headers and body. Reading the vat's own is not enough: a subclass can override `url`, and undici on Node 22 keeps the state behind it in a configurable own property — so a vat can both make it answer differently on each read and leave a `URL` object of its own in it to mutate while the caveat is still running
  - `harden()` does not close any of this: it freezes the wrapper, not the vat's argument
