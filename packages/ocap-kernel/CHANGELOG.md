...
- Garbage-collection action delivery now moves the kernel's own c-list, orphans an object whose owner has retired it, and releases the kernel's side only where the endpoint is gone for good — a terminated vat or a remote — rather than committing a release the endpoint was never told about ([#1010](https://github.com/MetaMask/ocap-kernel/pull/1010))
