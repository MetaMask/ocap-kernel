...
- **SECURITY:** The URL a vat's `fetch` requests is now always the URL its `network.allowedHosts` caveat approved. The allowlist and the `file:` prohibition were checked against one resolution of the caller's input, and then the input itself was forwarded to `fetch`, which resolved it a second time — so an input that answered differently on each read was validated as an allowed host and requested as a forbidden one (CWE-367). A vat holding `fetch` for a single host could reach any host, including loopback and cloud metadata endpoints ([#1026](https://github.com/MetaMask/ocap-kernel/pull/1026))
  ...
  - Unchanged, and still true: the caveat sees only the pre-flight URL, so a redirect from an allowed host to a disallowed one is followed. Vats that fetch from hosts they do not control should be treated accordingly
