...
- A delivery to a vat between workers waits for the new incarnation instead of resolving a live vat as a dead one, and a failed relaunch marks the vat terminated so its c-list can be reclaimed ([#1010](https://github.com/MetaMask/ocap-kernel/pull/1010))
  - Adds `VatManager.provideVat()`, the waiting counterpart to `getVat()`. Still open: a crank started in the turn between `waitForCrank` resolving and the restart being recorded can be mid-delivery when the worker goes down
