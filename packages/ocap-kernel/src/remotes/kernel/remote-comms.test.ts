...
describe('remote-comms', () => {
  ...
  describe('cross-incarnation wake detection', () => {
    it('passes crossIncarnationWake true when lastActiveTime gap exceeds threshold', async () => {
      ...
      expect(mockPlatformServices.initializeRemoteComms).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({ crossIncarnationWake: true }),
        mockRemoteMessageHandler,
        undefined,
        undefined,
        undefined,
    ...
    it('passes crossIncarnationWake false when no lastActiveTime exists', async () => {
      await initRemoteComms(
        mockKernelStore,
        mockPlatformServices,
        mockRemoteMessageHandler,
      ...
      expect(mockPlatformServices.initializeRemoteComms).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({ crossIncarnationWake: false }),
        mockRemoteMessageHandler,
        undefined,
        undefined,
        undefined,
    ...
    it('passes crossIncarnationWake false when gap is within threshold', async () => {
    ...
    it('writes lastActiveTime to KV store on init', async () => {
    ...
    it('logs when cross-incarnation wake is detected', async () => {
      const mockLogger = {
        log: vi.fn(),
        error: vi.fn(),
      };
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'comms init: cross-incarnation wake detected',
      );
    });
  });
