...
export function getPromiseMethods(ctx: StoreContext) {
   ...
   * The queued messages should be buffered by the caller via enqueueSend.
   *
   * @param kpid - The promise to resolve.
   * @param rejected - Whether the promise is rejected.
   * @param value - The resolution value.
   * @returns An array of queued messages that were waiting on this promise,
   *          each as a tuple of [target, message] to be passed to enqueueSend.
   */
