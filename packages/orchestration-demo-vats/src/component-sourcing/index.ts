...
import {
  COMPONENT_SOURCING_PRICE_USD,
  COMPONENT_SOURCING_PROVIDER_TAG,
  COMPONENT_SOURCING_SERVICE_DESCRIPTION,
  makeComponentSourcingService,
} from './service.ts';
import {
  getRemotableSpec,
  makeContactEndpoint,
...
const SERVICE_NAME = 'ComponentSourcingService';
...
/**
 * Build the ComponentSourcing service vat root.
 *
  ...
  return makeDefaultExo(`${SERVICE_NAME}VatRoot`, {
    async bootstrap(_vats: Record<string, unknown>, services: Services) {
      ...
      const contact = makeContactEndpoint({
        ...
        description: COMPONENT_SOURCING_SERVICE_DESCRIPTION,
        remotableSpec,
        getContactUrl: () => contactUrl,
        expectedToken: registrationToken,
        providerTag: COMPONENT_SOURCING_PROVIDER_TAG,
        priceUsd: COMPONENT_SOURCING_PRICE_USD,
      });
