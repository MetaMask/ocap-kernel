...
describe('wallet vat', () => {
  ...
  describe('balance', () => {
    it('starts at the default initial balance ($10,000)', () => {
      expect(root.balance()).toBe(1_000_000);
    });
  });
  ...
  describe('deposit', () => {
    ...
    it('rejects a negative amount', () => {
      expect(() => root.deposit(-1)).toThrow(/non-negative integer/u);
    });

    it('rejects a non-integer amount', () => {
      expect(() => root.deposit(12.5)).toThrow(/non-negative integer/u);
    });
  ...
  describe('withdraw', () => {
    ...
    it('rejects zero-amount withdrawals', async () => {
      await expect(root.withdraw(0)).rejects.toThrow(/positive/u);
    });

    it('rejects negative amounts', async () => {
      await expect(root.withdraw(-5)).rejects.toThrow(/non-negative integer/u);
    });

    it('rejects non-integer amounts', async () => {
      await expect(root.withdraw(1.5)).rejects.toThrow(/non-negative integer/u);
    });
  ...
  describe('init', () => {
    ...
    it('rejects negative amounts', () => {
      expect(() => root.init(-1)).toThrow(/non-negative integer/u);
    });

    it('rejects non-integer amounts', () => {
      expect(() => root.init(1.5)).toThrow(/non-negative integer/u);
    });
  });
  ...
  describe('persistence across re-incarnation', () => {
    it('restores the balance from baggage', async () => {
      ...
      await firstRoot.withdraw(250_000);
      expect(firstRoot.balance()).toBe(750_000);
      ...
      expect(secondRoot.balance()).toBe(750_000);
    });
