...
import {
  ...
  makeSchematicGenerationService,
} from './service.ts';
import {
  getRemotableSpec,
  makeContactEndpoint,
...
const SERVICE_NAME = 'SchematicGenerationService';
...
/**
 * Build the SchematicGeneration service vat root.
 *
...
export function buildRootObject(
  ...
  return makeDefaultExo(`${SERVICE_NAME}VatRoot`, {
    async bootstrap(_vats: Record<string, unknown>, services: Services) {
      const serviceExo = makeSchematicGenerationService();
      ...
      const contact = makeContactEndpoint({
        ...
        description: SCHEMATIC_GENERATION_SERVICE_DESCRIPTION,
        remotableSpec,
        getContactUrl: () => contactUrl,
        expectedToken: registrationToken,
        providerTag: SCHEMATIC_GENERATION_PROVIDER_TAG,
        priceUsd: SCHEMATIC_GENERATION_PRICE_USD,
      });
