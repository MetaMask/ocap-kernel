...
import {
  RETAIL_LISTING_PRICE_USD,
  RETAIL_LISTING_PROVIDER_TAG,
  RETAIL_LISTING_SERVICE_DESCRIPTION,
  makeRetailListingService,
} from './service.ts';
import {
  getRemotableSpec,
  makeContactEndpoint,
...
const SERVICE_NAME = 'RetailListingService';
...
/**
 * Build the RetailListing service vat root.
 *
...
export function buildRootObject(
  ...
  return makeDefaultExo(`${SERVICE_NAME}VatRoot`, {
    async bootstrap(_vats: Record<string, unknown>, services: Services) {
      const serviceExo = makeRetailListingService();
      ...
      const contact = makeContactEndpoint({
        ...
        description: RETAIL_LISTING_SERVICE_DESCRIPTION,
        remotableSpec,
        getContactUrl: () => contactUrl,
        expectedToken: registrationToken,
        providerTag: RETAIL_LISTING_PROVIDER_TAG,
        priceUsd: RETAIL_LISTING_PRICE_USD,
      });
