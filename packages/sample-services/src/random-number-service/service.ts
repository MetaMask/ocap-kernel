 ...
 *
 * Prefers `crypto.getRandomValues` (available when the kernel-utils-tools
 * `crypto` endowment is plumbed through). Falls back to a counter-based
 * sequence in environments where no crypto source is endowed —
 * sufficient for a demo service but not for anything caller-observable.
 *
