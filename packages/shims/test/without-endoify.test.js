...
test.failing('tamed constructors', t => {
  t.plan(12);
...
test.failing('create', t => {
  const c = new Compartment();
  t.is(1, 1);
  t.is(c.evaluate('1+1'), 2);
});

test.failing('SES compartment does not see primal realm names', t => {
  const hidden = 1; // eslint-disable-line no-unused-vars
  const c = new Compartment();
  t.throws(() => c.evaluate('hidden+1'), { instanceOf: ReferenceError });
});

test.failing('SES compartment also has compartments', t => {
  const c = new Compartment();
  t.is(1, 1);
  t.is(c.evaluate('1+1'), 2);
...
// test.failing('SESRealm has SES.confine', t => {
...
test.failing('SES compartment has harden', t => {
  const c = new Compartment({
    globals: { a: 123 },
    __options__: true,
  });
...
// test.failing('SESRealm.SES wraps exceptions', t => {
//   const s = SES.makeSESRootRealm();
//   function fail() {
