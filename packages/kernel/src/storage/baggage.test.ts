...
describe('Baggage', () => {
  ...
  describe('createWeakCollection', () => {
    ...
    it('should handle reference counting in weak collections', async () => {
      const collection = await baggage.createWeakCollection('test');
  ...
  describe('delete', () => {
    it('should delete stored values', async () => {
      await baggage.set('key1', 'value1');
      await baggage.delete('key1');
      expect(await baggage.get('key1')).toBeUndefined();
    });

    it('should not throw when deleting non-existent keys', async () => {
      expect(await baggage.delete('nonexistent')).toBeUndefined();
    });

    it('should prevent deletion of BAGGAGE_ID', async () => {
      await expect(baggage.delete(BAGGAGE_ID)).rejects.toThrow(
        'Cannot delete protected key: baggageID',
      );
      expect(await mockStore.get(BAGGAGE_ID)).toBe(STORAGE_VERSION);
    });

    it('should prevent deletion of NEXT_COLLECTION_ID', async () => {
      await expect(baggage.delete(NEXT_COLLECTION_ID)).rejects.toThrow(
        'Cannot delete protected key: nextCollectionId',
      );
      expect(await mockStore.get(NEXT_COLLECTION_ID)).toBe(2);
    });

    it('should maintain deletion across baggage instances', async () => {
      await baggage.set('key1', 'value1');
      await baggage.delete('key1');
      const newBaggage = await Baggage.create(mockStore);
      expect(await newBaggage.get('key1')).toBeUndefined();
    });

    it('should allow setting new value after deletion', async () => {
      await baggage.set('key1', 'value1');
      await baggage.delete('key1');
      await baggage.set('key1', 'value2');
      expect(await baggage.get('key1')).toBe('value2');
    });
  });
});
