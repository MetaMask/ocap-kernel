...
describe('Collection', () => {
  ...
  let collection: Collection<string, unknown>;
  ...
  describe('complex values', () => {
    it.each([
      ...
      ['array', [1, 2, 3], [1, 2, 3]],
      ['date', new Date('2024-01-01'), new Date('2024-01-01').toISOString()],
      //   ['undefined', undefined, null],
      //   ['infinity', Infinity, null],
      //   ['nan', NaN, null],
    ])(
      'should handle JSON serialization of %s',
      async (key, value, expected) => {
        await collection.init(key, value);
        const retrieved = await collection.get(key);
