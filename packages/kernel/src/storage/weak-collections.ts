...
import type { StoredEntry } from './collections.js';
 ...
 */
export class WeakCollection<Value extends Json> {
  readonly id: number;
   ...
   */
  #makeRefCountKey(key: string): string {
   ...
   */
  async #getRefCount(key: string): Promise<number> {
   ...
   */
  async #incRefCount(key: string): Promise<number> {
   ...
   * @returns The new reference count
   */
  async #decRefCount(key: string): Promise<number> {
    const newCount = (await this.#getRefCount(key)) - 1;
    await this.#store.set(this.#makeRefCountKey(key), newCount);
    if (newCount <= 0) {
   ...
   */
  async init(key: string, value: Value): Promise<void> {
   ...
   */
  async addRef(key: string): Promise<void> {
    await this.#incRefCount(key);
  }
   ...
   */
  async removeRef(key: string): Promise<void> {
