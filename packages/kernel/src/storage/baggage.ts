...
export class Baggage {
  ...
  async get<Value>(key: string): Promise<Value | undefined> {
    return this.#collection.get<Value>(key);
  }
  ...
  async set<Value>(key: string, value: Value): Promise<void> {
    await this.#collection.init<Value>(key, value);
  }

  /**
   * Deletes a value from baggage's own storage.
   *
   * @param key - The key to delete
   */
  async delete(key: string): Promise<void> {
    const protectedKeys = [BAGGAGE_ID, NEXT_COLLECTION_ID];
   ...
   */
  async createCollection(label: string): Promise<Collection> {
    const id = await this.#getNextCollectionId();
    return new Collection(id, this.#store, label);
  }
   ...
   */
  async createWeakCollection<Value extends Json>(
    label: string,
  ): Promise<WeakCollection<Value>> {
    const id = await this.#getNextCollectionId();
    return new WeakCollection<Value>(id, this.#store, label);
  }
}
