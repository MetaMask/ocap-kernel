  ...
  api.registerTool({
    ...
    async execute(
      ...
      try {
        ...
        const dest = await resolveSwapToken(params.destToken, wallet, chainId);
        if ('error' in dest) {
        ...
        const qGasEstimate =
          typeof quote.gasEstimate === 'string' ? quote.gasEstimate : 'unknown';
  ...
  api.registerTool({
    ...
    async execute(
      ...
      try {
        ...
        const rAggregator =
          typeof result.aggregator === 'string' ? result.aggregator : 'unknown';
