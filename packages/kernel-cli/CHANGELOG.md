...
- **BREAKING:** `executeDBQuery`, `clearState`, and `terminateAllVats` are no longer served on the daemon control socket unless the daemon is started with `OCAP_DEV_MODE=true` ([#988](https://github.com/MetaMask/ocap-kernel/issues/988))
...
- `$OCAP_HOME` is created `0700` and the daemon socket `0600`, instead of inheriting the ambient umask. The directory mode is reapplied on every start, so an `$OCAP_HOME` created before this change is brought forward rather than keeping its old permissions ([#988](https://github.com/MetaMask/ocap-kernel/issues/988))
