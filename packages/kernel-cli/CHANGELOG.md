...
- The daemon registers a `languageModelService` kernel service at startup when `llm.json` is present in the OCAP home, backed by any Open /v1-compatible endpoint (`provider`, `baseUrl`, and an API key indirected through `apiKeyEnv` or `apiKeyFile`); subclusters request it by listing the service name in their cluster config
