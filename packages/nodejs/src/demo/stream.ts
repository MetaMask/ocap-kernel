 ...
 */
export const makeVatStreamReader =
  (vat: RemoteStreamVat) => (streamId: number) => {
    const streamReader = {
      async next() {
...
type StreamMaker = {
  makeStream: () => Stream;
  readStreamFacet: {
    streamReadNext: (id: number) => Promise<IteratorResult<unknown, unknown>>;
    streamReadThrow: (
      id: number,
      error: Error,
 ...
 */
export const makeStreamMaker = (): StreamMaker => {
  let counter = 0;
   ...
   */
  const getStream = (id: number): Stream => {
  ...
  const streamReadNext = async (
    id: number,
  ): Promise<IteratorResult<unknown, unknown>> => {
    const stream = getStream(id);
    return await stream.reader.next(undefined);
  };
  const streamReadThrow = async (
    id: number,
    error: Error,
  ): Promise<IteratorResult<unknown, unknown>> => {
    const stream = getStream(id);
