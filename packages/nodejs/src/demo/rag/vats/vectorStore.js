...
export function buildRootObject(vatPowers, parameters, _baggage) {
  ...
  const addDocuments = async (docs) => {
    logger.debug('addDocuments:docs', JSON.stringify(docs, null, 2));
  ...
  const makeSecrecyFilter =
    (secrecy = DEFAULT_QUERY_SECRECY) =>
    (doc) =>
      doc.metadata.secrecy <= secrecy;
  ...
  const makeDocumentView = (
    ...
    const query = async (topic, nResults) => {
      ...
      const results = await vectorStore.similaritySearchWithScore(
        topic,
        nResults < maxResults ? nResults : maxResults,
        filter,
      );
  ...
  return Far('root', {
    async init() {
      ...
      const chunks = await Promise.all(
        documents.map(async ({ path, secrecy }) => {
